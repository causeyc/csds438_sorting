...
void parallelShellSort(int *array, int array_length, int num_cpus){
	...
	int block_num;
	int gap_size;
	int local_gap_size; //int
	gap_size = array_length / 2;
	block_size = array_length / num_cpus;
	local_gap_size = gap_size / block_size; // no double
	for(block_num = 0; block_num < num_cpus; block_num++){
		...
		for(local_gap_size = gap_size / block_size; local_gap_size > 1; local_gap_size -= (int) floor((float) local_gap_size / 2.0)){   // for fix local_gap_size /= 2
		//	puts("hi");
		//	printf("%d\n", (int) floor((float) local_gap_size / 2.0));
			//float to_print;
			//to_print = local_gap_size - local_gap_size/2;
			//	printf("%d\n", to_print);
